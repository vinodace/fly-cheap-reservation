            ...
            <ul class="pfb_checklist_web1">
              <?php if (!empty($results["dictionaries"]["carriers"])): ?>
                  <?php foreach ($results["dictionaries"]["carriers"] as $code => $name): ?>
                      <?php $checkboxId = strtolower(str_replace(' ', '_', $name)); ?>
                      <li>
        ...
        <?php if (!empty($results["data"])): ?>
            <?php foreach ($results["data"] as $offer): ?>
              <?php 
                  ...
                  // 🔑 Get airline info from the first itinerary's first segment
                  if (!empty($offer["itineraries"][0]["segments"][0])) {
                      $firstSeg = $offer["itineraries"][0]["segments"][0];
              ...
              ?>
                <div class="flight-result-box <?= $count > $limit ? 'd-none extra-flight' : '' ?>" data-airline="<?= strtolower($airlineCode) ?>">
                    <div class="row">
                        <!-- Flight Details -->
                        <div class="col-md-9">
...
<!-- Airling FLigth filtering + show more -->
<script>
 // Filter Flight by Airline
document.addEventListener("DOMContentLoaded", () => {
    ...
    const limit = <?= $limit ?>; // PHP limit

    let showAll = false; // track Show More click
    ...
    function filterFlights() {
        ...
        flights.forEach((box, index) => {
            const airline = box.dataset.airline;
            const matches = selectedAirlines.length === 0 || selectedAirlines.includes(airline);

            if (matches) {
                if (showAll || shownCount < limit || box.classList.contains("show-manual")) {
                    box.style.display = "block";
                    box.classList.add("show-manual");
                    shownCount++;
                } else {
                    box.style.display = "none";
                    box.classList.remove("show-manual");
                    hiddenCount++;
                }
            } else {
                box.style.display = "none";
    ...
    checkboxes.forEach(cb => cb.addEventListener("change", () => {
        showAll = false; // reset Show More when changing filter
        filterFlights();
    }));
    ...
    document.getElementById("clearFilters").addEventListener("click", () => {
        ...
        showAll = false;
        flights.forEach((box, index) => {
            box.style.display = index < limit ? "block" : "none";
            box.classList.remove("show-manual");
        });
        showMoreBtn.style.display = flights.length > limit ? "block" : "none";
    });
